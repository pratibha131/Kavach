...
const Analyze = () => {
  ...
  return (
    <DashboardLayout>
      ...
      <div className="max-w-4xl mx-auto mt-8 border-t border-white/5 pt-4">
        <div className="flex items-center gap-2 p-3 rounded-lg bg-secondary/20 border border-white/5 font-mono text-[10px] text-muted-foreground/60 overflow-hidden">
          <span className="shrink-0 text-primary/60 uppercase font-bold tracking-widest">System Diagnostics</span>
          <span className="h-3 w-px bg-white/10 mx-1"></span>
          <span className="truncate">Active Endpoint: {import.meta.env.VITE_API_URL || "FALLBACK (localhost:8000)"}</span>
          <span className="ml-auto hidden sm:block">Status: Ready</span>
        </div>
      </div>
    </DashboardLayout>
  );
