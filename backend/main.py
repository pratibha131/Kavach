...
async def analyze_file(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    case_db = models.CaseDB(
        ...
        transcript=f"Simulated transcript segment extracted from binary mapping {actual_hash[:6]}..." if severity == "critical" else "",
        scamMatchesStr=json.dumps([{"pattern": "High-urgency generative signature detected", "source": "Threat Intel DB", "category": crime_category, "confidence": round(min(99.0, confidence + 4.2), 1)}]) if severity == "critical" else json.dumps([]),
        narrativeRisk=round(min(99.0, confidence + float(int(actual_hash[-2:], 16) % 15)), 1) if severity == "critical" else round(min(70.0, confidence + 15.0), 1) if severity == "suspicious" else round(confidence / 2, 1),
        keywordsStr=json.dumps(["fraud", "manipulation", crime_category.split()[0].lower()]),
