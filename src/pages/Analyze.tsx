...
import { useCases, buildCaseFromAnalysis } from "@/contexts/CaseContext";
...
const Analyze = () => {
  ...
  const runAnalysis = () => {
    ...
    setAnalyzing(true);
    setTimeout(() => {
      // Randomize result for demo variety
      const confBase = Math.random();
      const isDeepfake = confBase > 0.3;
      const isSuspicious = !isDeepfake && confBase > 0.15;
      const confidence = isDeepfake ? 70 + Math.random() * 28 : isSuspicious ? 35 + Math.random() * 30 : 5 + Math.random() * 20;
      const verdict = isDeepfake ? "DEEPFAKE DETECTED" : isSuspicious ? "SUSPICIOUS CONTENT" : "AUTHENTIC";

      const analysisResult = {
        verdict,
        confidence: +confidence.toFixed(1),
        faceSwap: isDeepfake ? 70 + Math.floor(Math.random() * 28) : Math.floor(Math.random() * 30),
        voiceClone: isDeepfake ? 60 + Math.floor(Math.random() * 30) : Math.floor(Math.random() * 25),
        synthetic: isDeepfake ? 50 + Math.floor(Math.random() * 30) : Math.floor(Math.random() * 20),
        fingerprint: "MFP-2024-" + Math.random().toString(36).substring(2, 8).toUpperCase(),
      };

      setResult(analysisResult);
      setAnalyzing(false);

      // Add to shared case context — updates all dashboards
      const newCase = buildCaseFromAnalysis(file!, analysisResult);
      addCase(newCase);
